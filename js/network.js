(function() {
  ...
  overlay.innerHTML = '<div class="lead-modal"><button class="lead-close" onclick="closeLead()">&times;</button><div class="lead-content"><i class="fas fa-gift" style="font-size:3rem;color:#0ea5e9;margin-bottom:1rem;"></i><h3>Recevez votre guide gratuit</h3><p style="color:#64748b;margin-bottom:1.5rem;">"Les 5 erreurs qui ruinent votre piscine" — offert !</p><form action="https://formspree.io/f/xgodnlkr" method="POST"><input type="hidden" name="sujet" value="Newsletter - Guide gratuit"><input type="email" name="email" placeholder="Votre email" required style="width:100%;padding:0.75rem;border:2px solid #e2e8f0;border-radius:10px;margin-bottom:1rem;font-size:1rem;"><button type="submit" class="btn btn-blue btn-block"><i class="fas fa-download"></i> Recevoir le guide</button></form><p style="font-size:0.75rem;color:#94a3b8;margin-top:0.75rem;">Votre email ne sera jamais partagé. Désabonnez-vous à tout moment.</p></div></div>';
  ...
  overlay.querySelector('.lead-modal form').addEventListener('submit', function() { setTimeout(closeLead, 1000); });
  setTimeout(function() { if (document.getElementById('lead-overlay')) { overlay.querySelector('.lead-close').addEventListener('click', closeLead); overlay.addEventListener('click', function(e) { if (e.target === overlay) closeLead(); }); } }, 100);
})();